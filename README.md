...
`tools/list` at `https://gateway.pipeworx.io/security-feeds/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/security_feeds_list_feeds \
  -H 'Content-Type: application/json' \
  -d '{"category":"security"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/security_feeds_list_feeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "security-feeds": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-security-feeds"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-security-feeds
```
...
```
ask_pipeworx({ question: "your question about Security Feeds data" })
```
